...
CMD ["sh", "-c", "if [ -z \"$DATABASE_URL\" ]; then echo 'ERROR: DATABASE_URL not set!'; exit 1; fi && bunx prisma migrate deploy && NODE_ENV=production bun prisma/seed.ts && bun run dist/index.js"]
